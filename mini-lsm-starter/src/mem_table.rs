...
/// Create a bound of `KeySlice` from a bound of `KeySlice`.
...
pub(crate) fn map_lower_bound(lower: Bound<&[u8]>) -> Bound<KeySlice> {
    match lower {
...
pub(crate) fn map_upper_bound(upper: Bound<&[u8]>) -> Bound<KeySlice> {
    match upper {
...
impl MemTable {
    ...
    pub fn put_and_get_size(&self, key: KeySlice, value: &[u8]) -> usize {
        let _ = self.put(key, value);
        let entry_size = key.raw_len() + value.len();
...
impl StorageIterator for MemTableIterator {
    ...
    fn next(&mut self) -> Result<()> {
        let kv = self.with_iter_mut(|iter| MemTableIterator::entry_to_item(iter.next()));
        self.with_mut(|x| {
            *x.item = kv;
        });
        Ok(())
